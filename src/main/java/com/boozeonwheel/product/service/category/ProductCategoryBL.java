...
public class ProductCategoryBL {
	...
	public HomeResult getProductCategory(int categoryId) {
		...
		if ((list != null && list.size() > 0)) {
			for (ProductCategory productCategory : list) {
				...
				taxonomy.setId(productCategory.getCategoryId());
				taxonomy.setName(productCategory.getCategoryName());
				taxonomy.setCount(1);
				taxonomy.setCurrent_page(1);
				taxonomy.setPages(1);
				Root root = new Root();
				root.setId(productCategory.getCategoryId());
				...
				List<ProductCategory> rootTaxonList = productCategoryRepository.findByParentId(productCategory.getCategoryId());
				...
				if (rootTaxonList != null && rootTaxonList.size() > 0) {
					...
					for (ProductCategory taxonListObj : rootTaxonList) {
						...
						taxon.setId(taxonListObj.getCategoryId());
						...
						List<ProductCategory> taxonList = productCategoryRepository.findByParentId(taxonListObj.getCategoryId());
						if(taxonList !=null && taxonList.size()>0) {
							...
							for (ProductCategory taxonObj : taxonList) {
								...
								taxonData.setId(taxonObj.getCategoryId());
								...
								List<ProductCategory> taxonListFinal = productCategoryRepository.findByParentId(taxonObj.getCategoryId());
								if(taxonListFinal !=null && taxonListFinal.size()>0) {
									...
									for (ProductCategory taxonObjFinal : taxonListFinal) {
										...
										taxonDataFinal.setId(taxonObjFinal.getCategoryId());
	...
	public Root getProductSubCategoryByParentIdAndCategoryId(int parentId,int categoryId) {

		Root root = new Root();
		List<ProductCategory> list = productCategoryRepository.findByCategoryId(categoryId);
		if ((list != null && list.size() > 0)) {
			for (ProductCategory productCategory : list) {
				root.setId(productCategory.getCategoryId());
				root.setName(productCategory.getCategoryName());
	...
	public List<CategoryDTO> findAllProductCategory(){
		List<ProductCategory> productCategoryList=productCategoryRepository.findDistinctCategoyByParentId();
